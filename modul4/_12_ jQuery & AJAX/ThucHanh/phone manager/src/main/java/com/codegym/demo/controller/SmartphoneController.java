...
public class SmartphoneController {
    ...
    public ModelAndView editSmartphonePage(@PathVariable Integer id){
        Smartphone smartphone = smartphoneService.equals(editSmartphonePage(id))
        if(smartphone != null) {
    ...
    @RequestMapping(value = "/edit/{id}", method = RequestMethod.POST,
            produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public boolean editSmartphone(@PathVariable Integer id, @RequestBody Smartphone smartphone){
        smartphone.setId(id);
        return smartphoneService.equals(editSmartphonePage(id));
    }
